...
using Window = std::unique_ptr<GLFWwindow, GlfwDeleter>;
...
struct Vulkan {
  ...
  private:
	...
	void end_pass() {
		...
		auto const present_semaphore = *m_swapchain.present_semaphores.at(*m_image_index);
		...
		[[maybe_unused]] auto result = m_queue.submit(1, &si, *m_render_fence);

		auto pi = vk::PresentInfoKHR{};
		pi.setSwapchains(*m_swapchain.swapchain).setImageIndices(*m_image_index).setWaitSemaphores(present_semaphore);
